...
namespace tandem_mapper::chaining {
    ...
    Chains get_chains(const Contig & target,
        ...
        for (const size_t en : index) {
            ...
            const size_t chain_range = [&chain_matches, &common_params] {
                const size_t query_range = chain_matches.back().query_pos + common_params.k -
                        chain_matches.front().query_pos;
                const size_t target_range = chain_matches.back().target_pos + common_params.k -
                                            chain_matches.front().target_pos;
            ...
            if ((score >= chaining_params.min_score) and
                    (chain_range >= chaining_params.min_chain_range) and
                    (uniq_kmers >= chaining_params.min_uniq_kmers)) {
    ...
    SemiIntervals get_intervals(const Chain & chain, const size_t k) {
        ...
        const Sequence query_seq = chain.query_strand == dna_strand::Strand::forward ?
                                   chain.query.seq:
                                   chain.query.RC().seq;
...
} // End namespace tandem_mapper::chaining