...
class SketchContig {
 ...
 public:
  SketchContig(const Contig& contig_,
               ...
               const int nhash,
               const int nthreads,
               const size_t chunk_size) : contig{contig_},
                                          hasher{hasher_},
                                          k{k_},
                                          cms(nthreads,
                                              sketch::cm::ccm_t(static_cast<int>(ceil(log2(static_cast<double>(max_cnt)))),
                                                                static_cast<int>(ceil(log2(std::exp(exp_base) * static_cast<double>(contig_.size()) / nthreads))),
                                                                nhash)) {
...
class SketchContigs {
 ...
 public:
  SketchContigs(const std::vector<Contig>& contigs,
                ...
                const size_t chunk_size) : contigs{contigs},
                                           hasher{hasher},
                                           k{k},
                                           max_cnt{max_cnt},
        ...
        for (int j = 0; j < size; ++j) {
          ...
          const kmer_type::KmerType kmer_type =
              tandem_mapper::kmer_index::kmer_type::get_kmer_type(fhash,
                                                                  ...
                                                                  ban_f,
                                                                  max_cnt);
          hashes_pos_th[j].is_solid = (kmer_type == kmer_type::KmerType::unique) or (kmer_type == kmer_type::KmerType::rare);

          hashes_pos_th[j].is_unique = (kmer_type == kmer_type::KmerType::unique);
        }
      };
