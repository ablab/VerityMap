...
namespace veritymap::kmer_index_builder::approx_canon {

class ApproxCanonKmerIndexer : public AbstractKmerIndexBuilder {
 public:
  ApproxCanonKmerIndexer(const RollingHash<Config::HashParams::htype> &hasher,
  ...
  [[nodiscard]] kmer_index::KmerIndex Build(const std::vector<Contig> &contigs) const override {
    ...
    for (const Contig &contig : contigs) {
      ...
      while (true) {
        Config::HashParams::htype hash = kwh.hash();
    ...
    kmer_index::KmerIndex::KmerCounter counter;
    ...
    for (const Contig &contig : contigs) {
      ...
      for (kmer_index::kmer_window::KmerMinimizerWindow window(kwh, kmer_indexer_params.k_step_size, cms);
           kwh.hasNext();
           kwh = kwh.next(), window.Add(cms.est_count(kwh.hash()), kwh.hash(), kwh.get_fhash(), kwh.pos)) {
