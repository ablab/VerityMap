...
class KmerWindow {
    ...
    typename std::vector<std::tuple<size_t, htype, bool>>::const_iterator
        next_it;
    typename std::vector<std::tuple<size_t, htype, bool>>::const_iterator
        cur_it;
    ...
    void IncRight() {
        for (const size_t pos = std::get<0>(*cur_it);
             next_it!=pos_hash_uniq.cend();
             ++next_it) {
 ...
 public:
    KmerWindow(const size_t length,
    ...
    [[nodiscard]] double UniqueFrac() const {
        return tot_uniq/double(half_length*2);
    }
