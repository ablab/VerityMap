...
namespace tandem_mapper::kmer_index::sketch_contigs {
    ...
    public:
        SketchContig(const const std::vector<Contig> & contigs_,
    ...
    public:
        ...
        [[nodiscard]] std::vector<tandem_mapper::kmer_index::KmerIndex> get_kmer_indexes(const Counter & read_counter,
            ...
            for (auto [itcontig, itsc] = std::pair{contigs.cbegin(), sketch_contigs.cbegin()};
                ...
                while(true) {
                    const htype fhash = kwh.get_fhash();
                    const htype rhash = kwh.get_rhash();
                    const htype chash = kwh.hash();
                    const KmerType kmer_type = [&read_counter, &fhash, &rhash, &chash, &sketch_contig, this]() {
                        if (ban_filter.contains(fhash)) {
                        ...
                        if (fcnt == 1 && (read_counter.contains(chash) && read_counter.at(chash) > 35)) {
                            std::cout << fcnt << " " << read_counter.at(chash);
                            return KmerType::frequent;
                        }
                        if (fcnt == 1 && read_counter.contains(chash)) {
                            std::cout << fcnt << " " << read_counter.at(chash) << std::endl;
                        }
    ...
    template <typename htype>
    std::vector<KmerIndex> get_rare_kmers_approx(const Counter read_counter,
                                                        const std::vector<Contig> & contigs,
                                                        const RollingHash<htype> & hasher,
        ...
        return sketch_contigs.get_kmer_indexes(read_counter, kmer_indexer_params.k_step_size,
